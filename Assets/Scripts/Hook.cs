...
public class Hook : MonoBehaviour {
    ...
    private void CreateRope() {
        ...
        vertices[0] = startPosition - new Vector3(transform.position.x, 0, transform.position.z);
        for (int i = 1; i < vertex + 1; i++) {//Start
            //(Sp + cos(a), Sp + sen(a), S+cos(Sr))
            vertices[i] = new Vector3(startPosition.x - transform.position.x + Mathf.Cos(step * (i - 1)) * thickness, startPosition.y + Mathf.Sin(step * (i - 1)) * thickness, startPosition.z - transform.position.z + Mathf.Cos(startRotation.y) * Mathf.Cos(step * (i - 1)) * thickness) ;
        }
        for (int i = vertex + 1; i < 2 * vertex + 1; i++) {//End
            vertices[i] = new Vector3(endPosition.x - transform.position.x + Mathf.Cos(step * (i - 1)) * thickness, endPosition.y + Mathf.Sin(step * (i - 1)) * thickness, endPosition.z - transform.position.z + Mathf.Cos(endRotation.y) * Mathf.Cos(step * (i - 1)) * thickness);
        }
        vertices[2 * vertex + 1] = endPosition - new Vector3(transform.position.x, 0, transform.position.z);
